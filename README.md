...
- 每次模型调用使用独立空目录、临时会话与只读沙盒
- 忽略用户 Codex 配置和规则，并关闭 Codex 自带网页搜索、图片工具、Apps 与多 Agent
...
- 技能不能增加未授权工具，也不能跳过文件、剪贴板、网页和终端权限确认
...
- Codex 回复在 v1.1 通过非交互命令完成后一次显示，尚未使用 app-server 做逐字流式输出。
