...
form.onsubmit = async (event) => {
  ...
  if (mode === "jev") {
    if (!invokeCore) return;
    const apiKey = String(data.get("jevApiKey") || "").trim();
    try {
      await invokeCore("save_jev_provider_credentials", { apiKey });
      closeModal();
      await refreshJevProvider();
      await refreshDecisionShadowStatus();
    } catch (error) {
      window.alert(String(error));
    }
    return;
...
function renderRemoteSnapshot(snapshot) {
  ...
  const benchmarkPolicyRow = decisionShadow.benchmarkPolicyVersion
    ...
    : [];
  const providerHealthRow = providerHealth
    ? ['<div class="row"><strong>typesafe-jev</strong><span>health</span><div><span>'+
       (providerHealth.lastError && providerHealth.lastErrorAt &&
        (!providerHealth.lastSuccessAt || providerHealth.lastErrorAt > providerHealth.lastSuccessAt)
          ? 'last call failed'
          : 'last call healthy')+
...
async function refreshJevProvider() {
  ...
  try {
    const status = await invokeCore("get_jev_provider_status");
    jevConnectBtn.classList.toggle("hidden", status.configured);
    jevDisconnectBtn.classList.toggle("hidden", !status.configured);
  } catch {}
}
if (jevDisconnectBtn) {
  jevDisconnectBtn.onclick = async () => {
    if (!invokeCore) return;
    if (!window.confirm("断开 Jev 后，Rule Baseline 会继续独立运行。确定断开吗？")) return;
    try {
      await invokeCore("delete_jev_provider_credentials");
      await refreshJevProvider();
      await refreshDecisionShadowStatus();
    } catch (error) {
      window.alert(String(error));
