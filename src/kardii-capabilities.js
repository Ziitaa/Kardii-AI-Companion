(() => {
  ...
  const FEATURES = [
    ...
    },
    {
      ...
      group: "资料与工作",
      title: "连接企业微信",
      summary: "扫码连接有权访问的企微文档；API 模式机器人支持流式聊天，并可在本人绑定和二次确认后运行安全只读远程任务。",
      steps: [
      ...
      promptFact: "Kardii 的企业微信连接使用官方 wecom-cli v1.1.0：扫码账号只能搜索当前用户有权访问的文档，可读取和修改 doc / smartpage；搜索多候选必须由用户选择，写入前重新读取最新内容，创建/追加/覆盖每次确认，发布态 b1_ 智能文档只读。大圆或同事创建且分享给当前账号的文档可被搜索，但 Kardii 不能读取大圆的私有记忆或无权访问的文档。API 模式智能机器人收到消息后先显示正在思考，再在同一条消息中流式更新；可跟随桌面模型或单独选择 Gemini、DeepSeek、Codex、Ollama，并可切换快速或完整模式。机器人按 Bot、单聊与群聊隔离本机会话，普通聊天不带入桌面私人长期记忆或自定义指令，也不会自动执行 Agent。手机远程 Agent 默认关闭，只接受电脑生成的一次性码绑定的本人私聊；每个任务必须用 /任务 和第二个一次性 /确认 明确开始。首版只允许公开搜索及电脑单独授权的知识库只读、企微文档只读；终端、剪贴板、桌面浏览器、MCP、私人长期记忆和外部写入永久禁止。结果可直接回企微或用 /结果 取回。Bot Secret 保存在系统凭据库。",
    },
    {
