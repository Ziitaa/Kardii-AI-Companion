(() => {
  ...
  const FEATURES = [
    ...
    },
    {
      ...
      group: "资料与工作",
      title: "连接企业微信",
      summary: "扫码连接有权访问的企微文档；API 模式机器人支持附件和模型切换，并可在本人绑定和二次确认后运行安全只读远程任务。",
      steps: [
        "打开工作台的“外部连接”，在企业微信连接中扫码授权文档账号。",
        "搜索文档时先选择明确候选；Agent 的搜索和读取可自动，创建、追加和覆盖每次确认。",
        "要在企业微信中聊天，创建 API 模式智能机器人并保存 Bot ID 与 Secret，再选择专用模型及快速或完整模式。",
        "需要手机远程任务时，在电脑端单独开启并生成绑定码；企微私聊可发 /任务，明确的本机文件请求也可直接说，之后都要用一次性 /确认 开始。",
        "需要本机资料或 MCP 时，在电脑端预先选择只读目录，并逐项勾选已验证为只读的 MCP 工具。",
      ],
      example: "搜索企业微信里关于项目周报的文档，列出候选让我选择后再读取。",
      promptFact: "Kardii 的企业微信连接使用官方 wecom-cli v1.1.0：扫码账号只能搜索当前用户原本有权访问的文档，可读取和修改 doc / smartpage；大圆或同事创建并分享给当前账号的文档可以读取，但不能读取大圆的私有记忆或未分享文档。搜索多候选必须由用户选择，写入前重新读取最新内容，创建/追加/覆盖每次确认，发布态 b1_ 智能文档只读。API 模式智能机器人支持文字、已转写语音、图片和支持的文档附件，回答可使用企业微信媒体上传通道返回文件或图片；可跟随桌面模型或单独选择 Gemini、DeepSeek、Codex、Ollama，绑定用户也可用 /模型 切换电脑端已经配置好的企微专用模型。机器人按 Bot、单聊与群聊隔离本机会话，普通聊天不带入桌面私人长期记忆或自定义指令，也不会自动执行 Agent。手机远程 Agent 默认关闭，只接受电脑生成的一次性码绑定的本人私聊；绑定账号可先用 /任务 创建草稿，也可直接发送包含电脑或授权目录以及明确发送、查找、读取动作的文件请求，由本地固定规则创建草稿；两者都必须再用不带任务码的一次性 /确认 才会开始。远程任务允许公开搜索，以及电脑单独授权的知识库只读、企微文档只读、预先选择目录只读和逐工具 MCP 只读白名单；另有独立关闭的单文件回传开关，开启后已确认任务最多向当前绑定账号发送一个授权目录中的明确文件或 PNG/JPG 图片，禁止整个目录、批量文件、压缩包、可执行文件和未授权文件；MCP 后端会在调用时重新验证只读。终端、剪贴板、桌面浏览器、私人长期记忆、未授权文件、写入、删除、付款和购买始终禁止。结果可直接回企微或用 /结果 取回，长结果以 Markdown 文件返回。Bot Secret 保存在系统凭据库。",
    },
    {
  ...
  const VERSION_HIGHLIGHTS = [
    "新增手机企微远程 Agent：电脑端本人绑定、任务二次确认、固定只读白名单，并可在企微取回结果或补充回答。",
    "企业微信机器人新增即时思考提示、同消息流式更新、专用模型选择，以及快速与完整两种回复模式。",
  ...
  const LIMITATIONS = [
    ...
    "企业微信普通聊天只处理文字和已转写语音且不会自动执行；显式远程 Agent 仅限绑定账号二次确认的只读白名单，不是远程桌面，也不能创建或修改外部内容。只有电脑端另行开启文件回传后，才可按任务向绑定账号发送一个授权目录中的明确文件。",
